...
void do_remap(long num_nodes)
{
  unsigned long i, interchange, interchange_flag;
  // Use the current time for seeding the RNG
  struct timeval current_time;
  gettimeofday(&current_time, NULL);
  srand48(current_time.tv_usec);
  
  // Allocate the remap vector
  remap_vector = (unsigned long *)malloc(num_nodes*sizeof(unsigned long));
  if(remap_vector == NULL) {
    fprintf(stderr, "Unable to allocate remap vector in memory\n");
    exit(-1);
  }
  ...
  for(i=1; i<num_nodes; i++) {
    // Should we just slot it in ?
    interchange_flag = ((int)(drand48()*(i + 1)) > 0);
    // P(interchange_flag is true) = 1/(i+1)
    if(interchange_flag) {
      interchange = (int)(drand48()*i);
      // P(interchange == j) = 1/j for j = 0 to i-1
      remap_vector[i] = remap_vector[interchange];
      remap_vector[interchange] = i;
    }
    else {
      remap_vector[i] = i;
    }
