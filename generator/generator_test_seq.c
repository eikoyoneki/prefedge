...
int main(int argc, char* argv[]) {
  ...
  if (argc >= 3) edgemult = atoi(argv[2]);
  /* Start of graph generation timing */
  printf("# Nodes: %" PRIu64 " Edges: %" PRIu64 "\n", 1l << log_numverts, edgemult << log_numverts);
  printf("# NodeId \tNodeId\n");
  start = get_time();
  make_graph(log_numverts, edgemult << log_numverts, 1, 2, &nedges, &result);
