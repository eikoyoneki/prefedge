...
static void *prefetcher(void *arg)
{
  ...
  while(prefetch_thread_state != TERMINATE) {
    memset(laf, 0, 4*laf_size*sizeof(unsigned long));
...
//	fprintf(stderr, "Get %ld %ld %ld %ld\n", laf[i], laf[i+(1*laf_size)], laf[i+(2*laf_size)], laf[i+(3*laf_size)]);
	    retval = posix_fadvise(prefetch_fd,
				   offset*ASSUME_PAGE_SIZE, 
				   (laf[i+(2*laf_size)]+1) * ASSUME_PAGE_SIZE, 
				   POSIX_FADV_WILLNEED);
	    if(retval) {
 ...
 fprintf(stderr, "Get %ld %ld %ld %ld\n", laf[i], laf[i+(1*laf_size)], laf[i+(2*laf_size)], laf[i+(3*laf_size)]);
	    ...
	    if(aux_fd != -1) {
	      ...
	      retval = posix_fadvise(aux_fd,
				     ...
				     (laf[i+(3*laf_size)]+1) * ASSUME_PAGE_SIZE, 
				     POSIX_FADV_WILLNEED);
	      if(retval) {
		perror("Prefetch failed: ");
 fprintf(stderr, "Get %ld %ld %ld %ld\n", laf[i], laf[i+(1*laf_size)], laf[i+(2*laf_size)], laf[i+(3*laf_size)]);
		     ...
		     sequential_lookahead sequential_scan_function)
{
  ...
  laf = (unsigned long *)malloc(4 * laf_size * sizeof(unsigned long));
  assert(laf != NULL);
