		   ...
		   unsigned long vertex_cnt)
{
  ...
  while(forest != ULONG_MAX) {
	...
	if(first_child) { /* find child with max weight */
	  unsigned long *pmax = &dfs_metadata[root].children;
	  unsigned long max_weight = 0;
	  unsigned long *pnext = &dfs_metadata[root].children;
	  while((temp = *pnext)!= ULONG_MAX) {
	    if(dfs_metadata[temp].weight > max_weight) {
	      max_weight = dfs_metadata[temp].weight;
	      pmax = pnext;
	    }
	    pnext = &dfs_metadata[temp].child_next;
	  }
	  root = *pmax;
	  *pmax = dfs_metadata[root].child_next;
	}
	else {
...
int main(int argc, char **argv)
{
  ...
  for(i=0;i<vertex_cnt;i++) {
    ivert = dfs_metadata[i].stack_next;
    double converted_coord = (double)vertex_map[ivert].x_coord;
    converted_coord/=1000000.0;
    converted_coord=converted_coord*(M_PI/180); /* radians */
    keep_compiler_happy = write(fd_co, &converted_coord, sizeof(double));
    assert(keep_compiler_happy == sizeof(double));
    converted_coord = (double)vertex_map[ivert].y_coord;
    converted_coord/=1000000.0; 
    converted_coord=converted_coord*(M_PI/180); /* radians */
    keep_compiler_happy = write(fd_co, &converted_coord, sizeof(double));
    assert(keep_compiler_happy == sizeof(double));
    ...
    for(j=0;j<vertex_map[ivert].degree;j++) {
      double converted_weight = (double)weights[j];
      converted_weight/=1000; /* convert to km */
      keep_compiler_happy = 
	write(fd_rew, &converted_weight, sizeof(double));
      assert(keep_compiler_happy == sizeof(double));
    }
    rew_index += vertex_map[ivert].degree*sizeof(double);
    target_vertices--;
    if(target_vertices == 0) {
      CLOCK_STOP(periodic_clock);
