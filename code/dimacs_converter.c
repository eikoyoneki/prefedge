...
int main(int argc, char **argv)
{
  ...
  char *token;
  unsigned long i;
  uncompressed_dimacs_vertex_t *vertex_map;
  unsigned long current_offset;
  unsigned long keep_compiler_happy;
  unsigned long time_pass_vertex_info, time_pass1, 
    time_pass2, time_pass3, time_total;
  CLOCK_START(time_total);
  if(argc < 5) {
    fprintf(stderr, "Usage %s forward_graph reverse_graph co-ord output_graph\n",
	    argv[0]);
    exit(-1);
  }
  ...
  assert(strlen(argv[4]) < (1023 - 10));
  strcpy(string_buffer, argv[4]);
  ...
  while(fgets(string_buffer, 1024, fp_in) != NULL) {
...
#ifndef __APPLE__
	if(posix_fallocate(fd_vertices, 0, vertex_cnt*sizeof(uncompressed_dimacs_vertex_t))) {
	  perror("Unable to allocate space for vertices:");
	  exit(-1);
	}
#else
	for(i=0;
	    i<(vertex_cnt*sizeof(uncompressed_dimacs_vertex_t)/sizeof(unsigned long));
	    i++) {
...
#endif
	vertex_map =
	  (uncompressed_dimacs_vertex_t *)mmap(0, vertex_cnt*sizeof(uncompressed_dimacs_vertex_t),
	...
	if(mlock(vertex_map, vertex_cnt * sizeof(uncompressed_dimacs_vertex_t)) < 0) {
	  perror("mlocking vertex map failed:");
	}
	memset(vertex_map, 0, vertex_cnt*sizeof(uncompressed_dimacs_vertex_t));
      }
    }
    else {
      unsigned long from = atol(strtok(string_buffer," \t")) - 1;
      unsigned long to = atol(strtok(NULL," \t")) - 1;
  ...
  for(i=0;i<vertex_cnt;i++) {
    ...
    vertex_map[i].offset_edge_weights = current_offset;
    current_offset += vertex_map[i].degree*sizeof(unsigned long);
  ...
  while(fgets(string_buffer, 1024, fp_in) != NULL) {
    ...
    if(string_buffer[0] != '#') {
      ...
      unsigned long weight = atol(strtok(NULL," \t"));
      assert(from < vertex_cnt);
      assert(to < vertex_cnt);
      if(from == to) {
	continue;
	...
	&uncomp_alist_map[vertex_map[from].offset_edge_weights];
      if(dir == 'F') {
  ...
  while(fgets(string_buffer, 1024, fp_in) != NULL) {
	...
	&uncomp_alist_map[vertex_map[to].offset_edges];
      ...
      unsigned long* weights = (unsigned long *)
	&uncomp_alist_map[vertex_map[to].offset_edge_weights];
      if(dir == 'R') {
	/* unset bit for outgoing edge */
	props[BYTE_INDEX(eindex)] &=
	  ~(1 << BIT_INDEX(eindex));
  ...
  fprintf(stderr, "Pass...loading vertex info\n");
  CLOCK_START(time_pass_vertex_info);
  fclose(fp_in);
  ...
  while(fgets(string_buffer, 1024, fp_in) != NULL) {
    if(string_buffer[0] == 'c') {
      continue;
    }
    else if(string_buffer[0] == 'p') {
      continue;
    }
    else if(string_buffer[0] == 'v') {
      token = strtok(string_buffer, " ");
      unsigned long vert = atol(strtok(NULL, " ")) - 1;
      int x_coord = atoi(strtok(NULL, " "));
      int y_coord = atoi(strtok(NULL, " "));
      //printf("co-ord for %lu is %d:%d\n", vert, x_coord, y_coord);
      vertex_map[vert].x_coord = x_coord;
      vertex_map[vert].y_coord = y_coord;
    }
    else {
      fprintf(stderr, "Cannot recognise vertex info line %s\n",
	      string_buffer);
      exit(-1);
    }
  }
  CLOCK_STOP(time_pass_vertex_info);
  fclose(fp_in);
  close(fd_out);
  close(fd_vertices);
  close(fd_edges);
  ...
  printf("TIME PASS_VERTEX_INFO %lu\n", time_pass_vertex_info);
