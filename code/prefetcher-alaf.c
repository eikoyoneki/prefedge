...
/*	if ((alaf == 1) && (i == random_ift-1) && (entries < random_ift)) {
		laf_size *= 2; random_ift *= 2;
		free(laf);
		laf = (unsigned long *)malloc(2 * laf_size * sizeof(unsigned long));
  assert(laf != NULL);
		fprintf(stderr, "Extending LAF to %ld\n", laf_size);		
	}
*/
       }
       CLOCK_STOP(pftime);
	if ((alaf == 1)) {
       float iops = ((float) entries) / (((float) pftime) / 1000000.0f);
//	fprintf(stderr, "%f\n", iops);
	if ((iops < 5000) && (laf_size>128) ) {
                                laf_size /= 2; random_ift /= 2;
                free(laf);
                laf = (unsigned long *)malloc(2 * laf_size * sizeof(unsigned long));
  assert(laf != NULL);
//                fprintf(stderr, "Reducing LAF to %ld\n", laf_size);
        } else if ((iops > 5000) && (laf_size < 1024*1024)) {
                laf_size *= 2; random_ift *= 2;
                free(laf);
                laf = (unsigned long *)malloc(2 * laf_size * sizeof(unsigned long));
  		assert(laf != NULL);
//		fprintf(stderr, "Increasing LAF to %ld\n", laf_size);
	}
	}
    }
